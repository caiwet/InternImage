...
from .cxrlearn import cxrlearn as cxrlearn
...
# Added by us
class GloriaBlock(nn.Module):
    def __init__(self):
        super().__init__()
        os.chdir(os.path.join(os.getcwd(), "mmdet_custom/models/backbones/cxrlearn"))
        self.model = cxrlearn.gloria(
            model="resnet50",
        ...
        del self.model.global_embedder
        del self.model.local_embedder
        del self.model.pool
        del self.model.model.layer4
        print(self.model)


    def forward(self, x):
        global_features, local_features = self.model(x, get_local=True)
        return global_features, local_features


@BACKBONES.register_module()
class CustomInternImageCLS(nn.Module):
    r""" InternImage
    ...
    def forward(self, x):
        ...
        # compute gloria
        level = self.gloria
        gloria_global, gloria_local = level(x)

        x = self.patch_embed(x)
        x = self.pos_drop(x)

        # compute internimage and add to seq_out
